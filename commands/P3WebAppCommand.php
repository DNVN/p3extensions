...
EOD;
	...
	public function run($args)
	{
		...
		{
			$sourceDir=realpath(dirname(__FILE__).'/views/p3webapp');
			if($sourceDir===false)
				die("\nUnable to locate the source directory.\n");
			$list=$this->buildFileList($sourceDir,$path);
			$list['www/index.php']['callback']=array($this,'generateIndex');
			$list['www/index-test.php']['callback']=array($this,'generateIndex');
			$list['tests/bootstrap.php']['callback']=array($this,'generateTestBoostrap');
			$list['yiic.php']['callback']=array($this,'generateYiic');
			$this->copyFiles($list);
			@chmod($path.'/assets',0777);
			@chmod($path.'/runtime',0777);
			@chmod($path.'/data',0777);
			@chmod($path.'/data/testdrive.db',0777);
			@chmod($path.'/yiic',0755);
	...
	public function generateIndex($source,$params)
	{
		...
		$yii=$this->getYiiFrameworkPath('yii.php');		
	...
	protected function getYiiFrameworkPath($file=null){
		return realpath(dirname($_SERVER['SCRIPT_NAME'])).($file!==null?DIRECTORY_SEPARATOR.$file:'');
	}
}